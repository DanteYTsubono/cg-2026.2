...
let modoAtual = "RETA"; // Deafult RETA, ou TRIANGULO
let pontosClicados = [];
...
// 3. Geraçao das formas (RETA ou TRIÂNGULO)
function desenharForma(pontos) {
    let pixelsTotais = [];

    if (modoAtual === "RETA") {
        pixelsTotais = bresenham(pontos[0].x, pontos[0].y, pontos[1].x, pontos[1].y);
    } else if (modoAtual === "TRIANGULO") {
        const lado1 = bresenham(pontos[0].x, pontos[0].y, pontos[1].x, pontos[1].y);
        const lado2 = bresenham(pontos[1].x, pontos[1].y, pontos[2].x, pontos[2].y);
        const lado3 = bresenham(pontos[2].x, pontos[2].y, pontos[0].x, pontos[0].y);

        pixelsTotais = [...lado1, ...lado2, ...lado3];
    }

    // Pixels pra WebGL [-1.0, 1.0]
    const listaVertices = [];
    const listaCores = [];
    const listaTamanhos = [];

    for (let i = 0; i < pixelsTotais.length; i += 2) {
        const px = pixelsTotais[i];
        const py = pixelsTotais[i + 1];
...
canvas.addEventListener("mousedown", (event) => {
    ...
    pontosClicados.push({ x, y });

    if (modoAtual === "RETA" && pontosClicados.length === 2) {
        desenharForma(pontosClicados);
        drawScene();
        pontosClicados = []; // Reseta clicks
...
// 6. Interacao teclado - modo R e T, e Cores
window.addEventListener("keydown", (event) => {
    ...
    // Mmodo Triângulo
        ...
        modoAtual = "TRIANGULO";
        pontosClicados = []; // Limpa qualquer clique pendente
    } 
    // Troca de cores
    else {
        const num = parseInt(tecla);
        if (!isNaN(num) && num >= 0 && num <= 9) {
            corAtual = paletaCores[num];
...
// 7. Renderizaçao e iniciaçao
gl.clearColor(0.1, 0.1, 0.1, 1.0);

function drawScene() {
    gl.clear(gl.COLOR_BUFFER_BIT); // Apaga a figura anterior
    gl.useProgram(program);
    gl.drawArrays(gl.POINTS, 0, vertices.length / 2);
}

// Inicializa com a linha azul em (0,0) ate (0,0) no centro do canvas
const centroX = Math.round(canvas.width / 2);
const centroY = Math.round(canvas.height / 2);
desenharForma([{ x: centroX, y: centroY }, { x: centroX, y: centroY }]);
drawScene();